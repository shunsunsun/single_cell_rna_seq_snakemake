...
se <- readRDS(file="/gpfs2/gaog_pkuhpc/users/liny/GEJ_singleCell/data/GEJ_QCed_sctNorm_rescaleBatches_clustered.rds")
se <- DietSeurat(se, assays=c('RNA','SCT'), dimreducs='rspca')
sce <- as.SingleCellExperiment(se)

for(k in c(20,50,80)){
      print(paste0("snn neighborhood: ",k))
      g <- buildSNNGraph(sce, type="jaccard", k=k, use.dimred = 'RSPCA',BPPARAM=MulticoreParam(nworker))
      sce[[paste0("snn",k)]] <- igraph::cluster_walktrap(g)$membership
}
saveRDS(sce, file="/gpfs2/gaog_pkuhpc/users/liny/GEJ_singleCell/data/GEJ_QCed_sctNorm_rescaleBatches_walktrap_clustered.rds")
