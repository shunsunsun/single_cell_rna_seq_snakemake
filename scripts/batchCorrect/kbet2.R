...
useImmun=args[4]
if(useImmun=='T'){
	outfile <- gsub("kBET","kBETimmun",outfile)
}
set.seed(1129)
...
if(!file.exists(outfile)){
	...
	for(i in which(grepl("snn", colnames(df)) | grepl("_res.", colnames(df)))){
		...
		kBET_tmp <- foreach(j=1:length(uniq_c), .combine=rbind) %dopar% {
		...
		#save(kBET_tmp, file=gsub(".rds", paste0(clustMethod,".rds"), outfile))
