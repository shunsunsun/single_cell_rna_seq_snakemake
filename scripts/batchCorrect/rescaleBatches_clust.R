...
if(!file.exists(outfile)){
    ...
    corrected_se <- Seurat::RunPCA(as.matrix(assay(corrected,"corrected")), verbose = FALSE) ##By default nPC=50
    se[['rsbpca']] <- corrected_se[['pca']]
...
for (n in c(10,30,50)){
        for(k in c(20,50,80)){
                ...
                se <- RunUMAP(se, reduction='rsbpca',dims=1:n,reduction.name=paste0("UMAP_rsbpca",n,"_snn",k), reduction.key = paste0("UMAPrsbpca",n,"snn",k,"_"),n.neighbors=k)
                se <- FindNeighbors(se,reduction='rsbpca',dims=1:n,k.param=k,graph.name=paste0("rsbpca",n,"_snn",k))
