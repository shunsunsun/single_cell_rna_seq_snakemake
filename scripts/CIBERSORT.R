...
CoreAlg <- function(X, y, absolute, abs_method) {
    ...
    if (!absolute ||
        ...
        w <- (q / sum(q)) #relative space (returns fractions)
    if (absolute &&
...
CIBERSORT <-
    function(sig_matrix,
        ...
        if (dups > 0) {
            warning(paste(
                dups,
                " duplicated gene symbol(s) found in mixture file!",
                sep = ""
            ))
        ...
        header <-
            c('Mixture', colnames(X), "P-value", "Correlation", "RMSE")
        if (absolute)
            header <-
            c(header, paste('Absolute score (', abs_method, ')', sep = ""))
